...
cy_stc_sd_host_write_read_config_t sdcard_data = {
    .numberOfBlocks = BLOCK_BATCH_NUM,
...
void SAR_Interrupt() {
    ...
    if ((intr_status & (uint32_t) CY_SAR_INTR_EOS_MASK) == (uint32_t) CY_SAR_INTR_EOS_MASK) {
        ...
        if (write_idx == BUFFER_SIZE) {
            ...
            // Time to save the buffer
            if (record_mode) {
                sdcard_data.data = (uint32_t*) buffers[write_buffer_idx];
                sdcard_data.address += BUFFER_SIZE;

                Cy_SD_Host_Write(SDHC1, &sdcard_data, &sdHostContext);
            }
